...
{
  check_valid_address(f->esp, f->esp);
  ...
  switch(sys_code){
  	...
  	case SYS_EXEC:
  	{
  		...
  		check_valid_address(cmd_line, f->esp);
  		//printf("%p\n", cmd_line);
  		acquire_lock();
  		f->eax = exec(cmd_line);
  		release_lock();
  	...
  	case SYS_WAIT:
  	{
		...
		check_valid_address((int*)f->esp + 1, f->esp);
		int pid = *((int*)f->esp + 1);
		// printf("%d\n", pid);
		//acquire_lock();
		f->eax = wait(pid);
  	...
  	case SYS_CREATE:
  	{
		check_valid_address((unsigned*)f->esp + 2, f->esp);
  		char* file = *((int*)f->esp + 1);

  		check_valid_address(file, f->esp);
	...
	case SYS_REMOVE:
	{
  	...
  	case SYS_READ:
  	{
		...
		//printf("check buffer\n");
		check_valid_address((unsigned*)f->esp + 3, f->esp);
		...
		for(unsigned int i = 0; i < size; i++){
			//printf("check_buffer %p\n",tempBuf);
			check_valid_address(tempBuf, f->esp);
			tempBuf++;
		}
		...
		//check_buffer(buffer);
		struct spt_entry* spt = look_for_entry(buffer);
		if(spt){
			if(!spt->writable){
				exit(-1);
			}
...
void check_valid_address(const void * addr, void * esp){
	...
	if(!flag){
		struct spt_entry * spt = look_for_entry(addr);
		bool load = false;
		if(spt){
			load = load_lazy(addr);
        } else if(esp - 32 <= addr){
        //if more than stack size 32 bytes from esp
			//printf("%p\n", addr);
			//printf("grow stack\n");
			load = grow_stack(addr);
			//printf("finished growing stack\n");
		} else{
			//printf("quit here\n");
			exit(-1);
		}
		...
		if(!load){
			//printf("couldnt lazy load or grow stack\n");
			exit(-1);
		}
