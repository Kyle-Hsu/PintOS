               ...
               thread_func *function, void *aux) 
{
  ...
  list_push_back (&running_thread()->children, &child->cs_elem);
  // printf("Parent thread: %s List size: %d\n", running_thread()->name, list_size(&running_thread()->children));
...
thread_exit (void) 
{
  ...
  while(iter != list_end(&thread_current()->children)){
    struct child_status *child = list_entry (iter, struct child_status, cs_elem);
    struct list_elem* next = list_next(iter);
    list_remove(iter);
    iter = next;

    child->curr_thread->parent = NULL;
    free(child);
  }
